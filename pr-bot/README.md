...
- Rust, Zig, and Cargo build tooling is installed directly on EC2. Untrusted
  ...
  operations.
- A human provisions a dedicated remote-benchmark foundation from the sibling
  [`benchmarks-remote/`](../benchmarks-remote/README.md) project. The bot
  consumes its cluster and dataset bucket but never creates, updates, or
  destroys them.
...
Provision a dedicated benchmark foundation from `benchmarks-remote/` before
...
[`../authorized-github-logins.txt`](../authorized-github-logins.txt), with one
lowercase login per non-empty line.
...
Only users listed in `authorized-github-logins.txt` can enqueue benchmark jobs.
Authorization does not require the controller token to have repository contents
write access.
