...
import { datafusionDistributedGitReference } from "../src/lib/git-reference";
...
function repository(): string {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "benchmark-git-reference-"),
  );
...
test("uses the checked-out branch as the result reference", () => {
  const root = repository();
  try {
    git(root, "switch", "-c", "feature/benchmark-label");
    assert.equal(datafusionDistributedGitReference(root), "benchmark-label");
  } finally {
