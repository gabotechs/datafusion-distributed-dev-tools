...
const TRINO_SCHEMAS: Record<string, Record<string, string>> = {
  tpch: {
    customer: `(
   c_custkey    bigint,
   c_name       varchar(25),
...
function trinoSchemaForTable(table: {
  suite: string;
  name: string;
  schema: string;
}): string {
...
export class TrinoRunner implements BenchmarkRunner {
  ...
  private async executeSingleStatement(
    ...
    const submitResponse = await fetch(`${this.options.url}/v1/statement`, {
      method: "POST",
      headers: {
...
if (require.main === module) {
  ...
  void runEngineBenchmark(new TrinoRunner(options));
}
